...
function onOpen() {
  ...
  if (true) {
    // 5-axis machine configuration - 5轴机床配置 注意 旋转轴选择
    var bAxis = createAxis({ coordinate: 1, table: false, axis: [0, 1, 0], range: [-139.00, 139.00] });
    var cAxis = createAxis({ coordinate: 2, table: true, axis: [0, 0, 1], offset: [cAxisOffset, 0, 0], range: [-10000000000.0, 10000000000.0], cyclic: true });
    machineConfiguration = new MachineConfiguration(bAxis, cAxis);
    setMachineConfiguration(machineConfiguration);
    optimizeMachineAngles2(0); // map tip mode - we compensate below  // 优化机器角度
  }
  // set output format 根据机床配置 设定禁止或者开启轴
  ...
  if (getProperty("writeTools")) {
    ...
    // 这段代码的作用是生成刀具表的注释信息，并将其输出到 CNC 程序中。
...
/**********************************************************************************************************/
// 确定刀具在给定方向上是否会与工件（或原材料，即 stock）发生交点
...
/**********************************************************************************************************/
// 圆弧插补（Circular Interpolation）的 G-code 指令 
...
function writeRetract() {
  ...
  // z must be retracted first z 必须先安全回缩
